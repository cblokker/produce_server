  ...
  describe "#call" do
    ...
    context 'when there is a match' do
      it "returns sellers matching the buyer's purchase history within the rolling period" do
        result = UserQueries::MatchSellerToBuyers.new(seller: seller_0).call
        expect(result).to contain_exactly(buyer_0, buyer_1)
      end

      it "returns sellers matching the buyer's purchase history within the rolling period" do
        result = UserQueries::MatchSellerToBuyers.new(seller: seller_2).call
        expect(result).to contain_exactly(buyer_0, buyer_1)
      end

      it "returns sellers matching the buyer's purchase history within the rolling period" do
        result = UserQueries::MatchSellerToBuyers.new(seller: seller_3).call
        expect(result).to contain_exactly(buyer_0, buyer_1)
      end
    end
    ...
    context 'when there is not a match' do
      it "returns no preferred sellers when orders are too spaced out" do
        result = UserQueries::MatchSellerToBuyers.new(seller: seller_1).call
        expect(result).to eq([])
      end
    end
  end
